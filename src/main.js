...
class HeatedDotCom {
    ...
    bindEvents() {
        ...
        document.getElementById('field-filter').addEventListener('change', (e) => this.filterLeaderboard(e.target.value));
    }
    ...
    async filterLeaderboard(field) {
        await this.loadLeaderboard(field);
    }

    async loadLeaderboard(field = 'all') {
        try {
            const leaderboardData = await db.getLeaderboard(field, 10);
